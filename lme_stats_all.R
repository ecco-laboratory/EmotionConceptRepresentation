...
boot_sd <- apply(as.matrix(boot_results$t), 2, sd)
cohen_d <- as.numeric(observed_contrasts) / (boot_sd*sqrt(num_subjects))
...
#brain to 2dTraj explained by TEM
data <- read.csv('./outputs/performance_partial_corr_brainToTEMto2dTraj_it32000.csv')
data$region <- as.factor(data$region)
data$corr_type <- as.factor(data$corr_type)
data$TEM_component <- as.factor(data$TEM_component)
data$Emotion <- as.factor(data$Emotion)
...
compute_contrasts <- function(model) {
  ...
  contrast_spec <- list("Partial Corr vs. Corr x Category vs. 2dTraj" = contrast_hc)
...
print('corr vs partial corr x category vs 2dTra')
observed_contrasts <- compute_contrasts(model)
boot_results <- bootMer(model, FUN = compute_contrasts, nsim = 10000, type = "parametric")
boot_sd <- apply(as.matrix(boot_results$t), 2, sd)
cohen_d <- as.numeric(observed_contrasts) / (boot_sd*sqrt(num_subjects))
...
# HC, ERC, vmPFC predicting MDS trajectory**
data <- read.csv('./outputs/performance_all_CateMDStraj_hcecvmpfc.csv')
data$region <- as.factor(data$region)
data$Emotion <- as.factor(data$Emotion)
...
# HC category vs MDS trajectory contrast
compute_contrasts <- function(model) {
  emm_interaction <- emmeans(model, ~ region * Emotion)
  mds_variables <- c("mds1", "mds2")
  category_emotions <- setdiff(unique(emm_interaction@grid$Emotion), mds_variables) 
  grid <- emm_interaction@grid  # the grid of factor levels
  n_rows <- nrow(grid)
  contrast_hc <- numeric(n_rows)
  contrast_vmpfc <- numeric(n_rows)
  ...
  for (i in 1:n_rows) {
    ...
    } else if (emotion %in% mds_variables) {
      if (region == "Hippocampus") {
        contrast_hc[i] <- -1/length(mds_variables)
      } else {
        contrast_hc[i] <- 0
      }
    }
    ...
    # for vmPFC 
    if (emotion %in% category_emotions) {
      if (region == "vmPFC_a24_included") {
        contrast_vmpfc[i] <- -1/length(category_emotions)
      } else {
        contrast_vmpfc[i] <- 0
      }
    } else if (emotion %in% mds_variables) {
      if (region == "vmPFC_a24_included") {
        contrast_vmpfc[i] <- 1/length(mds_variables)
      } else {
        contrast_vmpfc[i] <- 0
      }
    }
  }
  contrast_spec <- list("HC's Category vs. MDS trajectory" = contrast_hc,
                        "HC's Category vs. MDS trajectory" = contrast_vmpfc)
  contrast_result <- contrast(emm_interaction, contrast_spec)
  estimates <- c("HC_Category_vs_MDS" = summary(contrast_result)$estimate[1],
                 "vmPFC_Category_vs_MDS" = summary(contrast_result)$estimate[2])
...
print('HC & vmPFC category vs MDS trajectory contrast')
print(result)
...
compute_contrasts <- function(model) {
  ...
  for (i in 1:n_rows) {
    ...
    if (emotion %in% category_emotions) {
      ...
      if (region == "Hippocampus") {
        contrast_hc_vs_erc[i] <- -1 / length(mds_variables)
      } else if (region == "EntorhinalCortex") {
        contrast_hc_vs_erc[i] <- 1 / length(mds_variables)
      }
    }
    ...
    if (emotion %in% category_emotions) {
      ...
      if (region == "Hippocampus") {
        contrast_hc_vs_vmpfc[i] <- -1 / length(mds_variables)
      } else if (region == "vmPFC_a24_included") {
        contrast_hc_vs_vmpfc[i] <- 1 / length(mds_variables)
      }
    }
    ...
    contrast_spec <- list(
    "HC vs ERC x Category vs. MDS trajectory" = contrast_hc_vs_erc,
    "HC vs vmPFC x Category vs. MDS trajectory" = contrast_hc_vs_vmpfc)
