...
for s = 1:num_subjects
    try 
        ...
        for type = types
            if strcmp(type{1}, '2dtrajMDS')
                behTab = jsondecode(fileread(fullfile(folder_project, 'data', 'mds_2d.json')));
                folder_name = '2dtrajMDS';
            end
            category_names = fieldnames(behTab.(bids_task_names{1}))';
            % Output directory
            ...
            for t = available_movies
                ...
                try
                    ...
                    normative_self_report = struct2array(behTab.(bids_task_names{t}));
            ...
            for r = selected_regions
                ...
                elseif strcmp(region_names{r}, 'Amygdala')
                    output_file = fullfile(output_dir, sprintf('%sRatings_prediction_performance_generalized_across_movies_amygdala.csv', type{1}));
                elseif strcmp(region_names{r}, 'alERC') || strcmp(region_names{r}, 'pmERC')
                    output_file = fullfile(output_dir, sprintf('%sRatings_prediction_performance_generalized_across_movies_alpmERC.csv', type{1}));
                else
