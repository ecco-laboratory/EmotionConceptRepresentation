...
class Model(torch.nn.Module):
    ...
    def forward(self, walk, prev_iter = None, prev_M = None):
        ...
        for g, x, a in walk:
            ...
            L, M, g_gen, p_gen, x_gen, x_logits, x_inf, g_inf, p_inf = self.iteration(x, g, steps[-1].a, 
                                                                                      [tensor.to(self.hyper['device']) for tensor in steps[-1].M], 
                                                                                      [tensor.to(self.hyper['device']) for tensor in steps[-1].x_inf], 
                                                                                      [tensor.to(self.hyper['device']) for tensor in steps[-1].g_inf])
    ...
    def init_trainable(self):
        ...
        self.MLP_D_a = MLP([self.hyper['n_actions'] for _ in range(self.hyper['n_f'])],
                            ...
                            bias=[True, False], device=self.hyper['device']).to(self.hyper['device'])        
        ...
        self.MLP_sigma_g_path = MLP(self.hyper['n_g'], self.hyper['n_g'], activation=[torch.tanh, torch.exp], hidden_dim=[2 * g for g in self.hyper['n_g']], device=self.hyper['device']).to(self.hyper['device'])
        # MLP for standard devation of grounded location from retrieved memory sample        
        self.MLP_sigma_p = MLP(self.hyper['n_p'], self.hyper['n_p'], activation=[torch.tanh, torch.exp], device=self.hyper['device']).to(self.hyper['device'])
        ...
        self.MLP_mu_g_mem = MLP(self.hyper['n_g_subsampled'], self.hyper['n_g'], hidden_dim=[2 * g for g in self.hyper['n_g']], device=self.hyper['device']).to(self.hyper['device'])
        ...
        self.MLP_sigma_g_mem = MLP([2 for _ in self.hyper['n_g_subsampled']], self.hyper['n_g'], activation=[torch.tanh, torch.exp], hidden_dim=[2 * g for g in self.hyper['n_g']], device=self.hyper['device']).to(self.hyper['device'])
        ...
        self.MLP_mu_g_shiny = MLP([1 for _ in range(self.hyper['n_f_ovc'] if self.hyper['separate_ovc'] else self.hyper['n_f'])], 
                                  ...
                                  hidden_dim=[2*n_g for n_g in self.hyper['n_g'][(self.hyper['n_f_g'] if self.hyper['separate_ovc'] else 0):]], device=self.hyper['device']).to(self.hyper['device'])
        ...
        self.MLP_sigma_g_shiny = MLP([1 for _ in range(self.hyper['n_f_ovc'] if self.hyper['separate_ovc'] else self.hyper['n_f'])],
                                     [n_g for n_g in self.hyper['n_g'][(self.hyper['n_f_g'] if self.hyper['separate_ovc'] else 0):]],
                                     hidden_dim=[2*n_g for n_g in self.hyper['n_g'][(self.hyper['n_f_g'] if self.hyper['separate_ovc'] else 0):]], activation=[torch.tanh, torch.exp], device=self.hyper['device']).to(self.hyper['device'])
        # MLP for decompressing highest frequency sensory experience to sensory observation
        self.MLP_c_star = MLP(self.hyper['n_x_f'][0], self.hyper['n_x'], hidden_dim=20 * self.hyper['n_x_c'], device=self.hyper['device']).to(self.hyper['device'])
...
class MLP(torch.nn.Module):
        ...
        with torch.no_grad():
            ...
            for n in range(self.N):
                ...
                if type(input_value[n]) is torch.Tensor:
                    self.w[n][from_layer].weight.copy_(input_value[n].to(self.device))                    
                # If only a single value is provided: set that value everywhere
                else:
                    self.w[n][from_layer].weight.fill_(torch.tensor(input_value[n], device=self.device)) 
    ...
    def forward(self, data):
        ...
        for n in range(self.N):
            ...
            module_output = self.w[n][0](input_data[n].to(self.device))
...
class Iteration:
    ...
    def pred_true(self):
        observation = self.x.detach().cpu()
        predictions = [tensor.detach().cpu() for tensor in self.x_gen]
