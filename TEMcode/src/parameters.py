...
parser = ArgumentParser(
    prog='parameters.py',
    description='Parameters for TEM model',
...
parser.add_argument(
    ...
    '--env_file',
    type=str,
    help="Json file of the environment to train on"
)

args = vars(parser.parse_args())
n_actions_from_env_file = json.load(open(args['env_file'], 'r'))['n_actions']
"""
# Change:
# params['n_x']
# params['n_actions']
...
def parameters(n_iteration, n_actions, n_x, device):
    params = {}
    ...
    params['train_it'] = n_iteration #4000
    ...
    params['g_downsample'] = [torch.cat([torch.eye(dim_out, dtype=torch.float),torch.zeros((dim_in-dim_out,dim_out), dtype=torch.float)]).to(device) for dim_in, dim_out in zip(params['n_g'],params['n_g_subsampled'])]
    return params

# This specifies how parameters are updated at every backpropagation iteration/gradient update
def parameter_iteration(iteration, params, device):
