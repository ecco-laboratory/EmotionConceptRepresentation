...
@author: jacobb
"""
#sbatch run_sbatch.sh --env_file ./envs/11x11_square_obs45_act5.json --n_train_iterations 1000000 --load_existing_model False --nodelist gpu2 --gres gpu:2
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to the environment file(s)"
)
parser.add_argument(
    '-n_it',
    '--n_train_iterations',
...
parser.add_argument(
    ...
    '--load_existing_model',
    type=str,
    help="Whether to load an existing model or start from scratch"
)
# Optional arguments
parser.add_argument(
    '-model_date',
    '--existing_model_date',
    type=str,
    help="Date of the existing model"
)
parser.add_argument(
    '-model_run',
    '--existing_model_run',
    type=str,
    help="Run number of the existing model"
)
parser.add_argument(
    '-model_it',
    '--existing_model_final_saved_iteration',
    type=int,
    help="Final iteration of the existing model"
)

args = vars(parser.parse_args())
env_files = args['env_file']
n_it = args['n_train_iterations']
json_data = [json.load(open(env_file, 'r')) for env_file in env_files]
# Check if all envs have the same number of actions
if len(set([data['n_actions'] for data in json_data])) > 1:
    raise ValueError("All environments must have the same number of actions")
n_act = json_data[0]['n_actions']
# Check if all envs have the same number of observations
if len(set([data['n_observations'] for data in json_data])) > 1:
    raise ValueError("All environments must have the same number of observations")
n_obs = json_data[0]['n_observations']
...
load_existing_model = True if args['load_existing_model'] == "True" else False
if load_existing_model:
    # Choose which trained model to load
    date = args['existing_model_date'] # 2020-07-05 run 0 for successful node agent
    run = args['existing_model_run'] #'0'
    i_start = args['existing_model_final_saved_iteration'] #3999
    ...
    params = parameters.parameters(n_iteration=n_it, n_actions=n_act, n_x=n_obs, device=device)
    # Save parameters
...
# Ensure all environments are selected at least once if len(envs) < batch_size
if len(envs) < params['batch_size']:
    selected_envs = np.concatenate([envs, np.random.choice(envs, params['batch_size'] - len(envs))])
else:
    selected_envs = np.random.choice(envs, params['batch_size'], replace=False)
environments = [world.World(graph, randomise_observations=True, shiny=(params['shiny'] if np.random.rand() < params['shiny_rate'] else None)) for graph in selected_envs]
#environments = [world.World(envs[0], randomise_observations=False, shiny=(params['shiny'] if np.random.rand() < params['shiny_rate'] else None))] + [world.World(graph, randomise_observations=True, shiny=(params['shiny'] if np.random.rand() < params['shiny_rate'] else None)) for graph in np.random.choice(envs,params['batch_size']-1)]
...
for i in range(i_start, params['train_it']):
    ...
    eta_new, lambda_new, p2g_scale_offset, lr, walk_length_center, loss_weights = parameters.parameter_iteration(i, params, device=device)
    ...
    for step in forward:            
        ...
        for env_i, env_visited in enumerate(visited):
            if env_visited[step.g[env_i]['id']]:
                step_loss.append(loss_weights*torch.stack([l[env_i] for l in step.L]).to(device))
            else:
        ...
        step_loss = torch.tensor(0).to(device) if not step_loss else torch.mean(torch.stack(step_loss, dim=0).to(device), dim=0)
