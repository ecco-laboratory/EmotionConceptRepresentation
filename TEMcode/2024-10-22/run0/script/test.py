...
parser = ArgumentParser(
    prog='test.py',
    description='Analyse a trained TEM model and plot analysis results',
...
parser.add_argument(
    ...
    '--model_date',
    type=str,
    help="Date of the model to load (format: YYYY-MM-DD)"
)
parser.add_argument(
    '-run',
    '--run_number',
    type=str,
    help="Run number of the model to load (format: 0 or 1 or 2, ...)"
)
parser.add_argument(
    '-index',
    '--model_index',
    type=str,
    help="Index (i.e., which interation) of the model to load"
)
...
date = model_date # '2020-10-13' 
run = run_number # '0'
index = model_index # '32000'
