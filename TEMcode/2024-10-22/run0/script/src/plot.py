...
def plot_cells(p, g, environment, n_f_ovc=0, columns_pg=[10,10], figsize_pg=[(20, 30),(20,20)], dpi=600,save=None):
    # Run through all hippocampal and entorhinal rate maps, big nested arrays arranged as [frequency][location][cell]
    for cells, names in zip([p, g],['p','g']):
        columns = columns_pg[0] if names == 'p' else columns_pg[1]
        figsize = figsize_pg[0] if names == 'p' else figsize_pg[1]
        ...
        n_rows_f = np.cumsum([0] + [np.ceil(len(c[0]) * 1.0 / columns).astype(int) + 1 for c in cells])

        # Create subplots for cells across frequencies
        fig, ax = plt.subplots(nrows=n_rows_f[-1], ncols=columns, figsize=figsize, dpi=dpi)
        # Switch all axes off
        for row in ax:
            for col in row:
                col.axis('off')
        ...
        for f, loc_rates in enumerate(cells):
            ...
            ax[n_rows_f[f]+1, int(columns/2)].set_title(names + ('' if f < len(cells) - n_f_ovc else ' object vector ') + ' cells, frequency ' 
            ...
            for c in range(len(loc_rates[0])):
                ...
                row = int(n_rows_f[f] + 1 + np.floor(c / columns))
                col = int(c % columns)
        ...
        if save is not None:
            fig.savefig(save[0] if names == 'p' else save[1])
    return fig, ax
